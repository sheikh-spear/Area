...
export default class WidgetTinderRecs extends React.Component
{
  ...
  getLiked() {
    fetch(`http://${global.ip}:8080/api/tinder-getteaser`, {
      method: 'POST',
      headers: {
  ...
  render() {
    ...
    if (global.tinder_data === undefined) {
      ...
      if (this.state.data != undefined) {
        data = this.state.data.map((data) => {
          return(<View><Image style={styles.tinderImg} source={{uri: data.user.photos[0].processedFiles[0].url}}/>
              <Text>{data.user.name}</Text></View>);
        });
      } else {
        this.getLiked();
    ...
    return (
      ...
      <Text style={{color:'white', textAlign: 'center'}}>get last likes</Text>
        <Modal animationType={"slide"} transparent={false}
          visible={this.state.modalVisible}
          onRequestClose={() => {console.log("closingmodal");}}>
