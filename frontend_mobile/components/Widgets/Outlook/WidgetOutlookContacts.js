...
export default class WidgetOutlookContacts extends React.Component
{
  constructor(props) {
    super(props);
    this.state = {
  ...
  getEmails() {
    fetch(`http://${global.ip}:8080/api/outlook-contacts`, {
      method: 'GET',
      headers: {
  ...
  render() {
        ...
        thing = this.state.data.map((dat) => {
          return (
            <View style={styles.email}>
              <Text>{dat.givenName}</Text>
              <Text>surname: {dat.surname}</Text>
            </View>
          );
        });
      }
    ...
    return (
      <View style={styles.container}>
        <Text style={{color:'white', textAlign: 'center'}}>get Contacts</Text>
