...
export default class WidgetOutlookMail extends React.Component
{
  constructor(props) {
    super(props);
    this.state = {
  ...
  getEmails() {
    fetch(`http://${global.ip}:8080/api/outlook-get-mail`, {
      method: 'GET',
      headers: {
  ...
  render() {
        ...
        thing = this.state.data.map((dat) => {
          return (
            <View style={styles.email}>
              <Text>{dat.from.emailAddress.name}</Text>
    ...
    return (
      <View style={styles.container}>
        <Text style={{color:'white', textAlign: 'center'}}>get mails</Text>
